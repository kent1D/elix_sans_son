...
function genie_elix_sans_son($time){
	...
	if(!$id_document)
		$id_document = sql_getfetsel('documents.id_document','spip_documents AS `documents` INNER JOIN spip_documents_liens AS L3 ON ( L3.id_document = documents.id_document ) INNER JOIN spip_signes AS L4 ON ( L4.id_signe = L3.id_objet AND L3.objet="signe")',"(documents.statut = 'publie') AND (documents.taille > 0 OR documents.distant='oui') AND (L4.id_signe > 0) AND (documents.mode = 'document') AND (documents.media = 'video') AND (documents.hasaudio = 'oui')",'','','0,1');
